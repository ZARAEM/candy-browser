...
| Tab overview modes, hero layout and gestures | Hero cards keep their resting geometry while the pager draw area extends behind profile chrome, so upward dismiss drags remain visible up to the system-bar boundary. Address chrome stays above both entry and exit hero animations. | `ui/BrowserScreen.kt`, `ui/TabDismissPhysics.kt` |
