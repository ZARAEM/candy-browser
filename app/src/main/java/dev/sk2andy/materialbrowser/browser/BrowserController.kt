...
class BrowserController(private val activity: Activity) {
    ...
    val commandCookieScope: CommandCookieScope
        get() = when {
            !supportsMultipleProfiles() -> CommandCookieScope.AllWebViews
            selectedTab.isIncognito -> CommandCookieScope.PrivateProfile
            else -> CommandCookieScope.SharedRegularProfile
        }
    ...
    private fun cookieManagerFor(webView: WebView): CookieManager =
        WebViewProfileCookies.managerFor(webView) ?: CookieManager.getInstance()
